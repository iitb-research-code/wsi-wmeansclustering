...
class GridBarycenter(Barycenter):
    ...
    # create a meshgrid to be used to evalute the Sinkhorn potential and take the max
    def _inner_init(self, grid = torch.tensor([]), grid_step = 50, grid_margin_percentage=0.05, **params):
        if grid.size(0) == 0:

            margin = (self.min_max_range[0,:] - self.min_max_range[1,:]).abs()*grid_margin_percentage

            tmp_ranges = [torch.arange(self.min_max_range[0,i]-margin[i],self.min_max_range[1,i]+margin[i],\
                                       ((self.min_max_range[1,i]-self.min_max_range[0,i]).abs()+2*margin[i])/grid_step)\
                          for i in range(self.d)]

            tmp_meshgrid = torch.meshgrid(*tmp_ranges)

            self.meshgrid = torch.cat([mesh_column.reshape(-1,1) for mesh_column in tmp_meshgrid],dim=1)
    ...
    def _initDistanceMatrices(self):
        ...
        self.Cgridy_list = [self.Cgridy[:,sl_idx[i]:sl_idx[i+1]] for i in range(self.num_distributions)]
    ...
    def _updateDistanceMatrices(self,x_new,idx):
        ...
        self.Cxgrid[idx:idx+x_new.size(0),:].copy_(dist_matrix(x_new,self.meshgrid)/self.eps)
    ...
    def _argminGradient(self,potentials_distributions,potentials_bary_sym):
        ...
        tmpM = (potentials_distributions/self.eps + self.full_weights.log()).view(1,-1) - self.Cgridy
        tmpM_list = [tmpM[:,sl_idx[k]:sl_idx[k + 1]] for k in range(self.num_distributions)]
        ...
        for k in range(self.num_distributions):
            ...
            V.mul_( (tmpM_list[k]-Vmax_k).exp().sum(1).pow(self.mixing_weights[k]).view(-1,1))
            Vmax_k.mul_(self.mixing_weights[k])

            Vmax.add_(Vmax_k)


        tmpN = (potentials_bary_sym/self.eps + self.bary.weights.log()).view(1,-1) - self.Cxgrid.t()
        ...
        return torch.clone(self.meshgrid[idx_max,:])
...
class Barycenter:
    ...
    def _storeDistributions(self,distributions):
        ...
        self.full_support = torch.cat([nu.support for nu in distributions],dim=0)
        self.full_weights = torch.cat([nu.weights for nu in distributions],dim=0)
        ...
        if self.mixing_weights.size(0) == 0 or self.mixing_weights.size(0) != self.num_distributions:
            self.mixing_weights = torch.ones(self.num_distributions) * (1.0/self.num_distributions)

        # if some weight is negative, throw an error
        if (self.mixing_weights<0).sum()>0:
            warnings.warn("Warning! Negative weights assigned to barycenter distributions!")
        ...
        self.support_number_points = torch.tensor([nu.support_size for nu in distributions])
    ...
    def _initBary(self,bary):
        ...
        self.potential_bary = torch.zeros(self.bary.support_size * self.num_distributions, 1)
    ...
    def _initDistanceMatrices(self):
        ...
        self._bigC = torch.empty(x_max_size,self.full_support.size(0)+x_max_size)
    ...
    def _updateDistanceMatrices(self,x_new,idx):
        ...
        self.Cxy_list = [self.Cxy[:,sl_idx[i]:sl_idx[i+1]] for i in range(self.num_distributions)]


        # current support of the barycenter
        bary_supp = self.bary.support

        self.Cxy[idx:idx+x_new.size(0),:].copy_(dist_matrix(x_new,self.full_support)/self.eps)
        self.Cxx[idx:idx + x_new.size(0), :].copy_(dist_matrix(x_new, bary_supp) / self.eps)
        ...
        if x_new.size(0) != self.bary.support_size:
            self.Cxx[:, idx:idx + x_new.size(0)].copy_(dist_matrix(x_new, bary_supp).t() / self.eps)
    ...
    def _updatePotentialsContainers(self):
        ...
        if self.bary.support_size > self.potential_bary_sym.size(0):
            ...
            tmp_potential_bary = torch.zeros(self.bary.support_size * self.num_distributions, 1)
            for k in range(self.num_distributions):
                ...
                tmp_potential_bary[idx_pre:idx_next].copy_(self.potential_bary[idx_pre_old:idx_next_old])
    ...
    def _computeSinkhorn(self):
        ...
        sl_idx = self.support_location_indices
        tmpM_list = [tmpM[:, sl_idx[i]:sl_idx[i + 1]] for i in range(self.num_distributions)]
        ...
        for idx_itr in range(self.sinkhorn_n_itr):
            ...
            for idx_nu in range(self.num_distributions):
                tmpM_list[idx_nu].copy_(B_list[idx_nu] + α_log - Cxy_list[idx_nu])
            ...
            err = self.eps * (A - A_prev).abs().mean()  # Stopping criterion: L1 norm of the updates
        ...
        for idx_nu in range(self.num_distributions):
            ...
            s_a = self.full_weights[sl_idx[idx_nu]:sl_idx[idx_nu+1],:].view(-1)
            inner_tmp_func_val = inner_tmp_func_val + torch.dot(a,s_a)

            b = B_list[idx_nu].view(-1)

            inner_tmp_func_val = inner_tmp_func_val + torch.dot(b,self.bary.weights.view(-1))

            tmp_func_val = tmp_func_val + self.mixing_weights[idx_nu] * inner_tmp_func_val
    ...
    def _computeSymSinkhorn(self):
        ...
        for idx_itr in range(self.sinkhorn_n_itr):
            ...
            A.copy_( 0.5 * (A - lse( (A + α_log).view(1,-1) - self.Cxx) ) ) # a(x)/ε = .5*(a(x)/ε + Smin_ε,y~α [ C(x,y) - a(y) ] / ε)

            err = self.eps * (A - A_prev).abs().mean()  # Stopping criterion: L1 norm of the updates
            if err.item() < self.sinkhorn_tol: break

        A.copy_(- lse( (A + α_log).view(1,-1) - self.Cxx) )  # a(x) = Smin_e,z~α [ C(x,z) - a(z) ]

        A.mul_(self.eps)

        tmp_func_val = self.mixing_weights.sum() * torch.dot(A.view(-1),self.bary.weights.view(-1))
    ...
    def performFrankWolfe(self, num_itr = 1):
        ...
        for idx_itr in range(num_itr):
            ...
            new_support_point = self._argminGradient(potentials_distributions,potentials_bary_sym)
            ...
            self._updateDistanceMatrices(new_support_point.view(1,-1),self.bary.last_updated_idx)
